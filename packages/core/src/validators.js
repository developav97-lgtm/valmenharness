 ...
 */
export function validateText(value, label, options = {}) {
...
export function isPrintable(value) {
    for (const character of value) {
        if (character === ASCII_SPACE)
            continue;
        if (NON_PRINTABLE_RE.test(character))
            return false;
    }
    return true;
}
...
export function validateNullableText(value, label) {
    if (value === null)
        return;
...
export function validateNullableNonNegativeInteger(value, label) {
    if (value === null)
        return;
...
export function validateNullableNonNegativeNumber(value, label) {
    if (value === null)
        return;
    if (typeof value === "boolean" ||
        typeof value !== "number" ||
 ...
 * `resolveReference`.
 */
export function validateReference(value, label = "La referencia") {
    if (value === null || value === undefined)
        return;
    if (typeof value !== "string" || !BUILD_REFERENCE_RE.test(value)) {
        fail(`${label} debe ser commit:<sha40> o worktree:sha256:<sha256> en minúsculas.`, EXIT_REFERENCE);
    }
}
/**
 ...
 */
export function requireExactKeys(item, keys, label) {
 ...
 */
export function validateSequential(entries, prefix, label) {
    const actual = entries.map((entry) => entry["id"]);
    const expected = entries.map((_entry, index) => `${prefix}-${String(index + 1).padStart(3, "0")}`);
 ...
 */
export function rstripChars(value, chars) {
...
export function isPlaceholder(normalized, placeholders) {
