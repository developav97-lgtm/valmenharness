 ...
 */
import { LEGACY_SCHEMA_VERSION, SCHEMA_VERSION, normalizeEvidenceKind, } from "./contract.js";
 ...
 */
export function planMigration(ticket, today) {
    const updates = {};
    const reasons = [];
    ...
    const kinds = evidenceKinds(ticket).filter((kind) => kind !== "" && normalizeEvidenceKind(kind) === undefined);
    return {
 ...
 */
export function migrateTicketText(text, today, options = {}) {
    ...
    validateDocument(reparsed, options.expectedId === undefined ? {} : { expectedId: options.expectedId });
 ...
 */
export function assertValidAfterMigration(text, expectedId) {
