 ...
 */
import { MAX_POINTS, POINT_STATES, RISK_LEVELS, TERMINAL_POINT_STATES, } from "./contract.js";
import { fail } from "./errors.js";
import { requireExactKeys, requireList, validateIsoDate, validateNullableText, validateNullableNonNegativeInteger, validateNullableNonNegativeNumber, validateReference, validateSequential, } from "./validators.js";
...
export function validatePoints(points) {
    ...
    for (const point of points) {
        ...
        for (const key of ["title", "actual", "expected"]) {
 ...
 */
export function isCanonicalRelativePath(raw) {
    if (raw === "" || raw.includes("\\"))
        return false;
    if (raw.startsWith("/"))
        return false;
 ...
 */
export function validateRelatedTicketId(value, label) {
    if (typeof value !== "string") {
        fail(`${label} debe ser null o un ID.`);
    }
}
/** Valida el bloque `QA`. */
export function validateQa(entries) {
...
export function validateEvidence(entries, pointIds) {
    validateSequential(entries, "EVIDENCE", "Evidencia");
    for (const entry of entries) {
        const id = entryId(entry, "EVIDENCE");
        requireExactKeys(entry, ["id", "date", "kind", "description", "reference", "point_id"], id);
        validateIsoDate(entry["date"], `${id}.date`);
...
export function validateRetests(entries, pointIds) {
    validateSequential(entries, "RETEST", "Retests");
    for (const entry of entries) {
        const id = entryId(entry, "RETEST");
        requireExactKeys(entry, ["id", "date", "point_id", "result", "evidence", "po_confirmation"], id);
        validateIsoDate(entry["date"], `${id}.date`);
...
export function validateClosures(entries) {
...
export function validateAiUsage(entries) {
    ...
    for (const entry of entries) {
        ...
        validateNullableNonNegativeNumber(entry["estimated_cost_usd"], `${id}.estimated_cost_usd`);
...
export function validateEvents(entries) {
    ...
    for (const entry of entries) {
        ...
        validateIsoDate(entry["date"], `${id}.date`);
        for (const key of ["action", "actor", "details"]) {
            const value = entry[key];
            if (typeof value !== "string" || value.trim() === "") {
                fail(`${id}.${key} debe ser texto no vacío.`);
            }
 ...
 */
export function pointIdsRecordedInEvents(events) {
    const ids = [];
    for (const event of events) {
        if (event["action"] !== "point-added")
            continue;
...
export function nextPointId(events, points) {
    const identifiers = [
    ...
    if (identifiers.length === 0)
        return "POINT-001";
    const highest = Math.max(...identifiers.map((identifier) => {
