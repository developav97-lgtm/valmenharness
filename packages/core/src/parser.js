import { FRONTMATTER_BLOCK_RE, FRONTMATTER_FIELDS, FRONTMATTER_LINE_RE, SECTIONS, STRUCTURED_SECTIONS, } from "./contract.js";
 ...
 */
export function isSafePlainScalar(value) {
...
export function parseFrontmatter(text) {
    ...
    if (keys.length !== FRONTMATTER_FIELDS.length ||
 ...
 */
export function parseSections(text, bodyStart) {
    ...
    const expectedTail = SECTIONS.slice(1);
    let canonical = null;
    ...
    while ((candidate = descriptionRe.exec(afterRequest)) !== null) {
        ...
        if (headings.length === expectedTail.length &&
...
function hasDuplicateKeys(source) {
    ...
    while (index < source.length) {
        ...
        if (character === '"') {
            ...
            let lookahead = index;
            while (isWhitespace(source[lookahead]))
                lookahead += 1;
 ...
 */
export function parseBlocks(sections) {
    const blocks = {};
