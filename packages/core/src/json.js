 ...
 */
export function formatPythonNumber(value) {
    ...
    const [mantissa, exponente] = value.toExponential().split("e");
...
function render(value, depth, indent) {
    if (value === null)
        return "null";
    ...
    if (typeof value === "object") {
        ...
        const entradas = Object.entries(value).filter(([, item]) => item !== undefined);
        if (entradas.length === 0)
            return "{}";
        const items = entradas.map(([clave, item]) => `${sangria}${JSON.stringify(clave)}: ${render(item, depth + 1, indent)}`);
        return `{\n${items.join(",\n")}\n${cierre}}`;
    }
    throw new Error(`No se puede serializar un valor de tipo ${typeof value}: no existe en JSON.`);
}
/**
 ...
 */
export function formatJsonBlock(value, indent = 2) {
