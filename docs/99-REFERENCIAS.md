...
// OJO: `id` y `provider` NO son required — hay que leerlos de forma defensiva.
{
  ...
  "answers": {
    ...
    "riesgo": {
      "type": "score",
      "score": 2.4, // ← double, puede caer ENTRE niveles
      "confidence": 0.71,
      "probabilities": {},
      "legend": {},
    },
...
`legend: { 0: '…', 1: '…' }` en la _pregunta_. Eso no existe. Lo correcto es:
...
> _"Jev fits here because it doesn't generate text. It reads the state you give it and
> returns a probability per question, so the gate is a few numeric comparisons your code
> controls rather than another prompt to parse."_

> _"Each proposition is complete on its own and is either true or false of the state. Don't
...
> _"The thresholds are deliberately far apart so the human reviews only calls with
> probabilities in the middle."_

> _"After a few weeks of real traffic, look at which `review` decisions the human approved
...
> _"Store the whole `GateDecision` with the ticket. `reason` and `checks` are the audit record
> for the outcome."_
...
| Latencia y costo                                                 | **Confirmado**  | Medido: 777 ms y $0.0000315 por una llamada con 4 preguntas                      |
...
| El endpoint responde y devuelve el formato esperado              | **Confirmado**  | Llamada real: HTTP 200, `typesafe/jev-1.13-20260917`, proveedor TypeSafe         |
| `noul` es un número en [0,1]                                     | **Confirmado**  | Llamada real                                                                     |
| **Entiende instrucciones en español**                            | **Confirmado**  | Llamada real: 0.760 en español vs 0.760 en inglés — diferencia 0.000             |
| El modelo discrimina y no responde siempre alto                  | **Confirmado**  | Una proposición deliberadamente falsa devolvió 0.010                             |
| Que `~typesafe/jev-latest` resuelva al modelo esperado           | **Documentado** | El alias existe; se usó el ID explícito, no el alias                             |
...
**Ya no queda ninguna afirmación sin verificar.** Las tres pendientes se cerraron con una
llamada real el 2026-09-21, ejecutando `scripts/verify-jev.mjs`.

### El resultado que importa: el gate funcionó

La proposición `cubre_todos_los_criterios` devolvió **0.760**, no 0.99. Con los umbrales del
diseño (aprueba ≥0.90, bloquea ≤0.10) eso cae en la **banda de revisión**: un humano decidiría.

Y tiene razón. Los criterios del ejemplo piden cuatro cosas y el plan cubre tres: no incluye
una verificación concreta de que el LocalAgent sigue funcionando. Jev detectó el hueco.

Eso es exactamente el comportamiento diseñado: **el gate no aprobó un plan incompleto**. Un
modelo de chat al que se le pregunta "¿está bien este plan?" tiende a responder que sí; este
devolvió una probabilidad que obliga a mirar.

### Dos datos para la calibración

| Observación                                         | Implicación                                                                                                                  |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Español 0.760 vs inglés 0.760, diferencia **0.000** | El idioma no afecta la evaluación. Los gates se pueden escribir en español sin perder precisión.                             |
| Una proposición falsa devolvió **0.010**            | El modelo discrimina. No hay sesgo hacia probabilidades altas, que era el riesgo principal de usar un modelo como evaluador. |

| Medición       | Valor                                                       |
| -------------- | ----------------------------------------------------------- |
| Latencia       | 777 ms (la documentación decía <600 ms)                     |
| Coste          | $0.0000315 por 4 preguntas — **~$0.000008 por proposición** |
| Contexto usado | 751 tokens de entrada, 115 de salida                        |

A ese precio, un gate de 7 proposiciones cuesta **$0.00006**. Un año de gates para 1.000
tickets costaría **menos de $0.15**.
...
| **SDD** | **Spec-Driven Development** ✅ | Rama _opt-in_ con 10 fases y artefactos formales                   |
...
- **Heurística de ~400 líneas por tarea** — explícitamente **no** es un cap: _"not a task
  acceptance criterion, hard cap, counter-trigger, automatic stop, forced split, or RDD
  trigger"_; prohibido minificar u omitir tests para cuadrar.
...
4. **Review Workload Forecast.** Líneas de texto **literales** para que un guard determinista
   ...
   ```
   _"The plain-text lines are the guard contract."_
5. **Mechanical Copy Contract.** Verbatim:
   > _"File content MUST NEVER pass through the model's Read/Write path to be copied — a
   ...
   > command (`cp -R`, `mv`, or `git mv`), verified by a structural readback."_
   >
   > _"An empty `diff -r` (no differences) is the only passing evidence… A skipped or missing
   > `diff -r` also FAILS the phase — agent self-report is never sufficient."_
6. **Distinción `blockedReasons` vs `notes`:**
   > _"`notes` is a separate, always-present array of informational diagnostics. A non-empty
   > `notes` NEVER blocks anything… `blockedReasons` stays reserved for genuine blockers."_
7. **Envelope de retorno de subagente** y la regla de la última acción:
   > _"Your FINAL output MUST be text (the return envelope), NOT a tool call… When a
   > sub-agent's last action is a tool call, the parent agent receives only the tool result —
   > your text response (the actual analysis) is lost."_
...
_"Es barato si se decide al principio y carísimo de retrofitear."_
...
| Bloques JSON _fenced_ append-only                            | El agente añade, nunca reescribe. Garantía de auditoría.                                   |
...
| Routing de modelo como **recomendación, nunca autoridad**    | _"Un routing de modelo nunca concede autoridad ni sustituye un gate."_                     |
